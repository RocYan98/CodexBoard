...
export function windowsSkillDocument(contents) {
  ...
  const entry =
    "## 运行入口\n\n从本次加载的 SKILL.md 定位同目录下 scripts/taskctl.ps1，使用 PowerShell 调用；保持项目原有工作目录。路径是占位符，替换后执行。\n\n```powershell\n$TASKCTL = 'C:\\实际安装目录\\manage-codexboard\\scripts\\taskctl.ps1'\n& $TASKCTL --help\n& $TASKCTL health\n```\n\n包装器仅调用完整 Windows 安装目录内的 runtime\\bin\\node.exe 与内置 taskctl，无需全局 Node。当前测试安装器的默认位置是 `%LOCALAPPDATA%\\CodexBoard Windows Test`；其次探测 Program Files 下的同名目录。自定义位置通过 `$env:CODEXBOARD_APP_PATH` 指定；显式路径无效则报错，不回退。默认数据目录为 `%LOCALAPPDATA%\\CodexBoard\\data`，`$env:CODEXBOARD_DATA_DIR` 可覆盖；包装器不迁移数据。\n\n```powershell\n$env:CODEXBOARD_APP_PATH = 'D:\\Apps\\CodexBoard Windows Test'\n& $TASKCTL --help\n```\n\n也可从 cmd 使用同目录 taskctl.cmd。若系统执行策略阻止 PowerShell 脚本，说明实际提示并交由用户处理，不自动放宽策略或添加 Bypass。Windows 测试版可首次安装 Skill；已有目录不会被自动替换。\n\n";
  return Buffer.from(
